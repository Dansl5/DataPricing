...
mse
MeanP=mean(df$price)
MeanQ=mean(exp(df$logmove))
Elasticity=(MeanP/MeanQ)*(-1.60131)
Elasticity
```

To evaluate the model fit we look at the MSE which is 0.8228178. Furthermore we can see that R\^2 of this model is 0.2081. The elasticity of this model is -0.0002111209, this makes sense as we are looking at the elasticity of the orange juice as whole, and people who really like orange juice will keep buying it even if the price increases.
...
summary(reg3)
DomElast=MeanP/MeanQ* -3.92032
MinElast=MeanP/MeanQ*-2.65843
TroElast=MeanP/MeanQ*-2.13001 
DomElast
MinElast
TroElast
```
...
MSE_nodomtrain=mean((training$logmove-prednodomtrain)^2)
MSE_nodomtrain
